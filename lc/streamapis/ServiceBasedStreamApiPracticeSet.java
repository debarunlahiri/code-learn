...
public final class ServiceBasedStreamApiPracticeSet {
    ...
    public static Map<String, List<String>> q097AnagramGroups(List<String> words) {
        return words.stream().collect(Collectors.groupingBy(ServiceBasedStreamApiPracticeSet::sortedKey));
    }

    public static List<List<String>> q098AnagramGroupsWithMoreThanOneWord(List<String> words) {
        return words.stream()
                .collect(Collectors.groupingBy(ServiceBasedStreamApiPracticeSet::sortedKey))
                .values()
                .stream()
                .filter(g -> g.size() > 1)
                .toList();
